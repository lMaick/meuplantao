...
export async function updateShift(id:string,input:ShiftUpdate):Promise<Shift>{const u=await getAuthenticatedUserId();const {data,error}=await createClient().from("shifts").update(input).eq("id",id).eq("user_id",u).select("*").single();throwOnError(error);return data as Shift;}
export async function removeShift(id:string):Promise<void>{const u=await getAuthenticatedUserId();const {error}=await createClient().from("shifts").delete().eq("id",id).eq("user_id",u);throwOnError(error);}
