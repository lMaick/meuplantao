...
export async function createShift(input:ShiftInput):Promise<Shift>{const u=await getAuthenticatedUserId();const {data,error}=await createClient().from("shifts").insert({...input,user_id:u}).select("*").single();await throwOnError(error);return data as Shift;}
export async function updateShift(id:string,input:ShiftUpdate):Promise<Shift>{const u=await getAuthenticatedUserId();const {data,error}=await createClient().from("shifts").update(input).eq("id",id).eq("user_id",u).select("*").single();await throwOnError(error);return data as Shift;}
