...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-muted/30">
    ...
    {open && <div className="fixed inset-0 z-50 bg-black/30 lg:hidden" onClick={() => setOpen(false)}><div className="ml-auto flex h-full w-80 max-w-[88vw] flex-col bg-background p-5" onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between"><span className="font-semibold">Menu</span><Button size="icon" variant="ghost" aria-label="Fechar menu" onClick={() => setOpen(false)}><X /></Button></div><nav className="mt-6 space-y-1">{[...primary, ...more].map(({ href, label, icon: Icon }) => <Link onClick={() => setOpen(false)} key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-3 text-sm", active(href) && "bg-primary/10 text-primary")}><Icon className="size-4" />{label}</Link>)}<button className="mt-4 flex w-full items-center gap-3 border-t px-3 py-4 text-sm text-muted-foreground"><LogOut className="size-4" />Sair</button></nav></div></div>}
